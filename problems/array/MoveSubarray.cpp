...
void MoveSubArray(int arr[], int length, int index)
{
	...
	{
		printf("Array length invalid, or input parameters invalid...\n");
		return;
	}

	if (length == 1)
	{
		printf("Array contains just one item, skipping move operation...\n");
		return;
	}

	if (index == 0 || index == length - 1)
	{
		printf("Index located at top or tail, skipping move operation...\n");
		return;
	}

	printf("Before moving sub-array...\n");
